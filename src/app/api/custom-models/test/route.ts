...
export async function POST(req: NextRequest) {
  ...
  try {
    if (id) {
      ...
      // 测试未保存的字段（直接来自表单）
      const { name, modelId, baseURL, apiKey, keyProvider } = body
      if (!modelId) {
    ...
    const provider = createCustomProviderInstance(cmRecord, apiKeyResolved)
    const result = await import("ai").then((ai) =>
      ai.generateText({
        model: provider(cmRecord.modelId),
        prompt: "ping",
        maxOutputTokens: 8,
      })
    )
