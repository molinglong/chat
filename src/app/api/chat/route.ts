...
import { buildCustomModelDefinition, resolveApiKey, createCustomProviderInstance } from "@/lib/ai/custom-model"
...
export async function POST(req: NextRequest) {
  ...
  if (modelId.startsWith("custom:")) {
    ...
    provider = createCustomProviderInstance(cmRecord, apiKey)
  } else {
    modelDef = getModel(modelId)
    if (!modelDef) {
  ...
  if (shouldExtractReasoning) {
    ...
    // Apply reasoning extraction middleware for all models that might produce reasoning
